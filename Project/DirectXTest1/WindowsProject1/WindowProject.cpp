	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			if (dropFrame >= 4) {
				...
				for (int i = g_SnakeArray.size() - 1; i > 0; --i){ //g_BodyCount - 1; i > 0; --i) {
...
void CheckSnakeBodies() {
	for (int i = 1; i < g_SnakeArray.size(); ++i) {
...
void Render() {
	...
	if (SUCCEEDED(g_pD3DDevice->BeginScene()))
	{
		...
		// 2번째 박스
		DrawSprite(center, g_SnakeArray[0], 0);
		for (int i = g_SnakeArray.size() - 1; i >= 0; --i) {
			DrawSprite(center, g_SnakeArray[i], 0);
		}

		RECT rc; //RECT는 Win32안에 있는애라서 따로 다른것들 안써줘도 된다.		
		rc = { 0,0,640,480 };//SetRect(&rc, 0, 0, 640, 480);
